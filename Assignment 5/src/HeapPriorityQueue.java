...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void upheapRec( int j ) {
	  int p = parent(j); //get original j
	  upheapRecHelper(j,p);

  }  // end upheapRec
  protected void upheapRecHelper(int j, int parent) {
	  if(j <= 1) {
		  return;
	  }
	  parent = parent(j);
	  if(compare(heap.get(j), heap.get(parent)) >= 1) {
		  return;
	  }
	  swap(j,parent);
	  j = parent;
	  upheapRecHelper(j, parent);
  }
  ...
  protected void downheapRec( int j ) {
	  if(hasLeft(j)) {
		  ...
		  downheapRecHelper(j, left, smallChildIndex); 
	  }
   
  }  // end downheapRec
  protected void downheapRecHelper(int j, int leftIndex, int smallChild){
	  if( !(hasLeft(j)) ) {
		  return;
	  }
	  leftIndex = left(j);
	  smallChild = leftIndex;
	  if(hasRight(j)) {
		  int rightIndex = right(j);
		  if(compare(heap.get(leftIndex) , heap.get(rightIndex)) > 0) {
	  ...
	  swap(j, smallChild);
	  j = smallChild;
	  downheapRecHelper(j, leftIndex, smallChild);
  }
  ...
  public int updateKey( K currentKey, K newKey ) {
	...
	// You may use loops in this method.
	  ...
	  for(int  i = 1; i < this.size(); i ++) {
		  if(this.heap.get(i).getKey().equals(currentKey)) {
			  Entry<K,V> e = new PQEntry<K,V>(newKey,this.heap.get(i).getValue());
			  this.heap.set(i, e);
			  heapify(); //rebuild heap and check it again.
			  returner = i;
			  for(int j =1; j < this.size(); j++) {
				  if(this.heap.get(j).getKey().equals(newKey)) {
					  returner = j;
				  }
        ...
        return returner;    // replace this line with your code
